...
export default function CoachExperience() {
  ...
  const [personalDetails, setPersonalDetails] = useState({ fullName:"", phone:"", location:"", linkedinUrl:"", websiteUrl:"" });
  ...
  useEffect(() => {
    ...
    createClient().auth.getUser().then(async ({ data }) => {
      ...
      if (accountResponse.ok) setPersonalDetails({ fullName:account.full_name||"", phone:account.phone||"", location:account.location||"", linkedinUrl:account.linkedin_url||"", websiteUrl:account.website_url||"" });
  ...
  function chooseProfile(id) {
    ...
    setActiveProfileId(id);setResume(selected?.resume_text||"");setJobDescription("");setAnalysis(null);setAnswers([]);setAnswer("");setQuestionIndex(0);setDocuments(null);setRecommendationChoices({});setReviewStatuses({});setStage("input");
  }
  ...
  async function saveManagedProfile() {
    ...
    try {
      const response=await fetch("/api/profiles",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:selected.id,name:managedProfileName.trim(),targetRole:managedProfileRole.trim(),resume:selected.resume_text||""})});
      const result=await response.json().catch(()=>({}));
  ...
  async function savePersonalDetails() {
    ...
    try {
      ...
      setPersonalDetails({fullName:result.full_name||"",phone:result.phone||"",location:result.location||"",linkedinUrl:result.linkedin_url||"",websiteUrl:result.website_url||""});
  ...
  return <main className="min-h-screen bg-[#f4f2eb] text-ink">
    ...
    {accountOpen&&<div className="fixed inset-0 z-[80] flex justify-end bg-[#18201d]/35 backdrop-blur-sm" onMouseDown={e=>{if(e.target===e.currentTarget)setAccountOpen(false)}}><aside className="h-full w-full max-w-md overflow-y-auto bg-[#f4f2eb] p-6 shadow-2xl md:p-8" role="dialog" aria-modal="true" aria-label="Your profile"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#1f6650]">Your workspace</p><h2 className="mt-2 font-display text-4xl">Your profile</h2><p className="mt-2 text-sm leading-6 text-ink/50">Keep your contact details ready and manage separate career directions.</p></div><button onClick={()=>setAccountOpen(false)} aria-label="Close menu" className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-black/10 bg-white text-lg">×</button></div><div className="mt-7 rounded-[24px] border border-black/[.07] bg-white/70 p-5"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-[#18201d] font-display text-xl text-white">{(personalDetails.fullName||user?.email||"Y").charAt(0).toUpperCase()}</span><div><p className="font-semibold">Personal details</p><p className="text-xs text-ink/40">Used in your resumes and cover letters</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-xs text-ink/50 sm:col-span-2">Full name<input value={personalDetails.fullName} disabled={accountSaving} onChange={e=>setPersonalDetails({...personalDetails,fullName:e.target.value})} autoComplete="name" className="mt-1.5 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f6650]"/></label><label className="text-xs text-ink/50 sm:col-span-2">Email<input value={user?.email||""} disabled className="mt-1.5 w-full rounded-2xl border border-black/10 bg-black/[.03] px-4 py-3 text-sm text-ink/45"/></label><label className="text-xs text-ink/50">Phone<input value={personalDetails.phone} disabled={accountSaving} onChange={e=>setPersonalDetails({...personalDetails,phone:e.target.value})} inputMode="tel" autoComplete="tel" className="mt-1.5 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f6650]"/></label><label className="text-xs text-ink/50">Location<input value={personalDetails.location} disabled={accountSaving} onChange={e=>setPersonalDetails({...personalDetails,location:e.target.value})} autoComplete="address-level2" className="mt-1.5 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f6650]"/></label><label className="text-xs text-ink/50 sm:col-span-2">LinkedIn<input value={personalDetails.linkedinUrl} disabled={accountSaving} onChange={e=>setPersonalDetails({...personalDetails,linkedinUrl:e.target.value})} inputMode="url" placeholder="linkedin.com/in/your-name" className="mt-1.5 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f6650]"/></label><label className="text-xs text-ink/50 sm:col-span-2">Portfolio or website<input value={personalDetails.websiteUrl} disabled={accountSaving} onChange={e=>setPersonalDetails({...personalDetails,websiteUrl:e.target.value})} inputMode="url" placeholder="yourportfolio.com" className="mt-1.5 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f6650]"/></label></div>{profileError&&<p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-800">{profileError}</p>}<button disabled={accountSaving} onClick={savePersonalDetails} className="mt-5 w-full rounded-full bg-[#1f6650] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">{accountSaving?"Saving…":"Save personal details"}</button></div><button onClick={()=>{setAccountOpen(false);openProfileManager()}} className="mt-5 flex w-full items-center justify-between rounded-[22px] border border-black/[.07] bg-white/60 p-5 text-left"><span><strong className="block font-display text-xl">Career profiles</strong><small className="mt-1 block text-ink/45">{profiles.length} separate {profiles.length===1?"direction":"directions"}</small></span><span className="text-xl text-[#1f6650]">→</span></button><p className="mt-3 px-2 text-xs leading-5 text-ink/40">Each career profile has its own source resume and application workflow. Switching profiles starts a clean workspace.</p><button onClick={async()=>{await createClient().auth.signOut();location.href="/login"}} className="mt-7 w-full text-center text-xs text-ink/40 underline underline-offset-4">Sign out</button></aside></div>}
