...
export default function CoachExperience() {
  ...
  async function generate() {
    ...
    try {
      ...
      const data = await callCoach("generate", { resume, jobDescription, analysis, answers, positioning, acceptedRecommendations, declinedRecommendations });
  ...
  return <main className="min-h-screen bg-[#f4f2eb] text-ink">
    <header className="sticky top-0 z-20 border-b border-black/[.06] bg-[#f4f2eb]/85 backdrop-blur-xl"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><Logo/><div className="flex items-center gap-3 text-xs text-ink/45"><span className="hidden sm:inline">{user?.email||"Private by default"}</span>{user&&<button onClick={openProfileManager} aria-label="Open profile menu" className="grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white/70 transition hover:border-[#1f6650]/30"><span className="space-y-1"><i className="block h-px w-4 bg-ink/60"/><i className="block h-px w-4 bg-ink/60"/><i className="block h-px w-4 bg-ink/60"/></span></button>}</div></div></header>
