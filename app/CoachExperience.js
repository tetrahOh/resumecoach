...
export default function CoachExperience() {
  ...
  function editManagedProfile(id) {
    ...
    setManagedProfileId(id);setManagedProfileName(selected?.name||"");setManagedProfileRole(selected?.target_role||"");setProfileError("");setDeleteConfirmId("");
  ...
  async function deleteProfile(id) {
    setProfileSaving(true);setProfileError("");setProfileNotice("");
    try {
      const response=await fetch(`/api/profiles?id=${encodeURIComponent(id)}`,{method:"DELETE"});
      const result=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(result.error||"That profile could not be deleted.");
      const remaining=profiles.filter(profile=>profile.id!==id);
      setProfiles(remaining);setDeleteConfirmId("");
      if(activeProfileId===id){setActiveProfileId("");setResume("");setJobDescription("");setAnalysis(null);setAnswers([]);setAnswer("");setQuestionIndex(0);setPositioning("Problem solver");setDocuments(null);setRecommendationChoices({});setReviewStatuses({});setStage("input");setProfileHistory([]);}
      editManagedProfile(remaining[0]?.id||"");
      setProfileNotice("Career profile deleted.");
    } catch(e) { setProfileError(e.message); } finally { setProfileSaving(false); }
  }
