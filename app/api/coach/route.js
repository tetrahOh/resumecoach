...
const CATEGORY_FIT_SCHEMA={type:"object",additionalProperties:false,required:["Technical expertise","Problem solving","Communication","Leadership","Business impact"],properties:{"Technical expertise":{type:"integer",minimum:0,maximum:100},"Problem solving":{type:"integer",minimum:0,maximum:100},Communication:{type:"integer",minimum:0,maximum:100},Leadership:{type:"integer",minimum:0,maximum:100},"Business impact":{type:"integer",minimum:0,maximum:100}}};
const POSITIONING_OPTION_SCHEMA={type:"object",additionalProperties:false,required:["label","reason","evidence","resumeScore","roleScore","categoryFit"],properties:{label:{type:"string"},reason:{type:"string"},evidence:{type:"array",items:{type:"string"}},resumeScore:{type:"integer",minimum:0,maximum:100},roleScore:{type:"integer",minimum:0,maximum:100},categoryFit:CATEGORY_FIT_SCHEMA}};
...
const CUSTOM_POSITIONING = `You are ResumeCoach. The person wants to see themselves presented using a specific positioning label of their own choosing, instead of one of your suggested options. Address the person directly as "you" and "your". Evaluate this exact label honestly against their real resume and the target job description. Never invent evidence — cite only phrases genuinely present in the supplied resume. Be honest, not flattering: if the label is a weak or unsupported fit, say so plainly and let resumeScore, roleScore and categoryFit reflect that low fit rather than softening the reason. resumeScore describes how strongly the current resume already proves this story (0-100). roleScore describes how useful this story would be for this specific job (0-100). categoryFit breaks resumeScore down per employer priority category (Technical expertise, Problem solving, Communication, Leadership, Business impact), scoring 0-100 how strongly this label's evidence proves each one. Return only JSON: {"label":"the exact label as given","reason":"1-2 sentences on why this does or doesn't fit you, citing real evidence where it exists","evidence":["supporting evidence phrases, or an empty array if none exist"],"resumeScore":0,"roleScore":0,"categoryFit":{"Technical expertise":0,"Problem solving":0,"Communication":0,"Leadership":0,"Business impact":0}}.`;
